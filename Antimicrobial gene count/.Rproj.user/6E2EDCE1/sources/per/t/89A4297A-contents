...
df_human <- read_excel("MRSA.xlsx", sheet = "Human")
...
genesE_list <- df_human$`Complete MRSA genes detected` %>%
  na.omit() %>%
  strsplit(split = ",") %>%
  unlist() %>%
...
total_genesE3 <- sum(gene_counterE$Count)

# Calculate percentage
gene_counterE <- gene_counterE %>%
  mutate(Percentage = round((Count / total_genesE3) * 100, 1),
...
# Remove missing values and split genes
genes_listE2 <- df_human$`Complete MRSA genes detected` %>%
  na.omit() %>%
  strsplit(split = ",") %>%
  unlist() %>%
  trimws()  # Remove extra spaces
...
# Compute total gene count
total_genesE3 <- sum(gene_counterE2$Count)
...
Human_count  <- flextable(gene_counterE) %>%
  save_as_docx(gene_counterE, path = "Gene counts for Human.docx")

Human_count2  <- flextable(gene_counterE2) %>%
  save_as_docx(gene_counterE2, path = "Gene counts for Human2.docx")
